...
<uw-blockquote> Pasos: Hipótesis, medias, calcular SCT y SCE, gl, CM y F. Interpretar el resultado y concluir. 
---
## Ejemplo ANOVA - manual
```{r}
data = data.frame(
...
data_df = data %>% pivot_longer(., cols = everything(), names_to = "metodos", values_to = "notas")
summary(aov(lm(notas ~ metodos, data = data_df)))
...
| Prueba | Tipo de control del error | Nivel de conservadurismo | Cuándo se usa | Ventaja principal | Desventaja |
|------|------|------|------|------|------|
| Tukey (HSD) | Controla el error familiar (FWER) para todas las comparaciones | Alta (conservadora) | Cuando se comparan **todas las medias entre sí** después de ANOVA | Método estándar en biología; control estadístico robusto | Puede detectar menos diferencias |
| Newman–Keuls (SNK) | Control parcial del error tipo I | Intermedia | Cuando los tratamientos tienen **orden lógico** (ej. dosis crecientes) | Más poder que Tukey | No controla completamente el error familiar |
| Duncan | Control débil del error tipo I | Baja (liberal) | Análisis **exploratorios** o clasificación de tratamientos (común en agronomía clásica) | Detecta más diferencias entre grupos | Mayor riesgo de falsos positivos |
...
## ANOVA por bloques : 1 variable cuantitativa >2 grupos dependientes
...
- Hay diferentes ANOVAs dependiendo del diseño experimental. 
